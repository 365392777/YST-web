...
<template>
  <div>
        ...
        <tbody v-for="(tableData, index) in orders" :key="index">
          ...
          <tr>
                ...
                <tbody>
                  <tr v-for="tableDetail in tableData.orderDetails" :key="tableDetail.detailId">
                    ...
                    <td class="orderItem">
                      ...
                      <!-- 0--售后中 -->
                      <div class="orderIcon" v-if="tableData.orderDetails[0].serviceStatus==='售后中'">
                        <img src="../../../../assets/shouhouzhong@2x.png" alt>
                      </div>
                      <!-- 1--售后失败 -->
                      <div
                        class="orderIcon"
                        v-else-if="tableData.orderDetails[0].serviceStatus==='售后失败'"
                      >
                        <img src="../../../../assets/shibai@2x.png" alt>
                      </div>
                      <!-- 2--售后成功 -->
                      <div
                        class="orderIcon"
                        v-else-if="tableData.orderDetails[0].serviceStatus==='售后成功'"
                      >
                        <img src="../../../../assets/shouchenggong@2x.png" alt>
                      </div>
                    </td>
            ...
            <td>
              <el-button
                class="outline"
                @click="handleAfterSale(index)"
                v-if="tableData.orderDetails[0].serviceStatus==='售后中'"
              >售后处理</el-button>
        ...
        <el-tab-pane label="物流信息">
          <div class="print-body dialogDelivery">
            <el-form :inline="true" class="demo-form-inline">
              ...
              <el-form-item label="发货人：" prop="id">
                <template>{{ formDetail.id }}</template>
              </el-form-item>
    ...
    <!-- 售后处理 -->
    <el-dialog title="售后处理" :visible.sync="dialogDetail2" center :lock-scroll="false" width="60%">
      <div class="print-body">
        <div class="title">商品信息</div>
        <el-table :data="tablePrint" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleAfterDefe">售后失败</el-button>
        <el-button type="primary" @click="handleAfterSucc">售后成功</el-button>
      </span>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        // console.log(val);
        val.map(v => {
          this.orderServiceOneData.orderDtls.push(v.detailId);
        });
      },
      //售后失败---7
      handleAfterDefe() {
        this.orderServiceOneData.serviceStatus = 7;
        changeAfterSale(this.orderServiceOneData).then(res => {
          // console.log(res);
          if (res.data.error == "") {
      ...
      //售后成功---8
      handleAfterSucc() {
        this.orderServiceOneData.serviceStatus = 8;
        changeAfterSale(this.orderServiceOneData).then(res => {
          if (res.data.error == "") {
            this.$message.success("操作成功");
            setTimeout(() => {
